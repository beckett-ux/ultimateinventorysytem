...
export const getRequestOrigin = (request) => {
  ...
  const forwardedProtoRaw = readHeader("x-forwarded-proto");
  const forwardedHostRaw = readHeader("x-forwarded-host");
...
export const getShopifyApiKey = () => {
  const apiKey = process.env.SHOPIFY_API_KEY;
  const publicKey = process.env.NEXT_PUBLIC_SHOPIFY_API_KEY;

  if (apiKey && publicKey && apiKey !== publicKey) {
    const error = new Error("SHOPIFY_API_KEY and NEXT_PUBLIC_SHOPIFY_API_KEY must match");
    error.code = "INVALID_ENV";
    error.env = "SHOPIFY_API_KEY";
    throw error;
  }

  const resolved = apiKey || publicKey;
  if (!resolved) {
    const error = new Error(
      "Missing required env var: SHOPIFY_API_KEY (or NEXT_PUBLIC_SHOPIFY_API_KEY)"
    );
    error.code = "MISSING_ENV";
    error.env = "SHOPIFY_API_KEY";
    throw error;
...
export const getShopifyAppUrlOrigin = () => {
  ...
  try {
    ...
    const error = new Error(`Invalid SHOPIFY_APP_URL: ${appUrl}`);
    error.code = "INVALID_ENV";
...
export const getShopifyOAuthCallbackUrl = (options = {}) => {
  const override = process.env.SHOPIFY_OAUTH_CALLBACK_URL;
  if (override) {
    try {
      return new URL(override).toString().replace(/\/$/, "");
    } catch {
      const error = new Error(`Invalid SHOPIFY_OAUTH_CALLBACK_URL: ${override}`);
      error.code = "INVALID_ENV";
      error.env = "SHOPIFY_OAUTH_CALLBACK_URL";
      throw error;
    }
  }
  ...
  const origin =
    options.origin ||
    (options.request ? getRequestOrigin(options.request) : null) ||
    getShopifyAppUrlOrigin();

  const url = new URL(SHOPIFY_OAUTH_CALLBACK_PATH, origin);
  return url.toString().replace(/\/$/, "");
};

export const tryGetShopifyOAuthCallbackUrl = (options = {}) => {
  try {
    return getShopifyOAuthCallbackUrl(options);
  } catch {
    return null;
  }
};

export const tryGetShopifyAppUrlOrigin = () => {
...
export function getShopify() {
  ...
  const appUrlOrigin = getShopifyAppUrlOrigin();
  const appUrlParsed = new URL(appUrlOrigin);
